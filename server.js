      ...
      .aggregate([
        { $sort: { fecha_venta: -1 } },
        {
          $group: {
            ...
            total_vendido: {
              $sum: { $multiply: ["$cantidad", "$precio_venta"] }
            }
          }
        }
      ])
...
app.post("/agregar_sabor", function(req, res) {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    dbo
      .collection("sabores")
      .findOne({ sabor: req.body.sabor }, function(err, resp) {
        if (resp) {
...
app.post("/agregar_cliente", function(req, res) {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    dbo
      .collection("clientes")
      .findOne({ cliente: req.body.cliente }, function(err, resp) {
        if (resp) {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    ventas
      .aggregate([
        { $match: { categoria: { $exists: true } } },
        { $unwind: "$cliente" },
        { $match: {$or : [{cliente : cliente0 }, {cliente : cliente1}]}},
        {
          $group: {
            _id: {
              cliente: "$cliente",
		...
		// A continuacion se hace un promedio de los promedios entre los dos equipos
		{
			$group: {
			  _id: {
				sabor: "$_id.sabor",
			  ...
			  promedio_vendido: { $avg: "$promedio_vendido" }
			}
		  }
      ])
      .toArray(function(err, docs) {
